...
export const EditableElementHeading1 = (props: EditableElementHeading1Props): JSX.Element => {
  if (props.isEditing) {
    return <>
      <Heading1><Input maxLength={64} disabled={props.isWorking} value={props.inputValue} onKeyDown={(event: React.KeyboardEvent<HTMLInputElement>) => { console.log(event); }} onChange={(event: React.ChangeEvent<HTMLInputElement>) => props.onChange(event.target.value)} /></Heading1>
      <ButtonContainer>
